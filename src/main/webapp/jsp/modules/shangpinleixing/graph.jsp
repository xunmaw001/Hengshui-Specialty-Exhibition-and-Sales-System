...
<body>
	...
	<script >
		...
		$.ajax({ 
			type: "GET",
			url: baseUrl+"value/shangpinleixing/"+xColumnName+'/'+yColumnName,
			data:{ },
		...
		function group(colName){
			$.ajax({ 
                type: "GET",
                url: baseUrl+"group/shangpinleixing/"+colName,
             	data:{	tableName: "shangpinleixing",
                    	columnName: colName
					 },
                beforeSend: function(xhr) {
                    xhr.setRequestHeader("token", window.sessionStorage.getItem('token'));
